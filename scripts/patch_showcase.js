...
console.log('Original personal_brand_v4.html length:', html.length);

// 1. CSS ENHANCEMENTS
// Replace .snk-light-spot with enhanced lighting, particle canvases, odometer, and 3D exploded styles
...
if (!html.includes(targetCss)) {
  throw new Error('targetCss not found in html');
}
html = html.replace(targetCss, replacementCss);
console.log('✓ Replaced CSS');

// 2. PROJECT 1 HTML: Add petals canvas in #portal-viewport
...
if (!html.includes(targetPortalCanvas)) {
  throw new Error('targetPortalCanvas not found');
}
html = html.replace(targetPortalCanvas, replacementPortalCanvas);
console.log('✓ Added portal-petals-canvas');

// 3. PROJECT 2 HTML: Sneaker HUD, tilt card particles & full angle pills
...
if (!html.includes(targetSneakerHud)) {
  throw new Error('targetSneakerHud not found');
}
html = html.replace(targetSneakerHud, replacementSneakerHud);
console.log('✓ Replaced sneaker turntable HUD');
...
if (!html.includes(targetSnkCard)) {
  throw new Error('targetSnkCard not found');
}
html = html.replace(targetSnkCard, replacementSnkCard);
console.log('✓ Added snk-particles-canvas to tilt card');
...
if (!html.includes(targetSnkPills)) {
  throw new Error('targetSnkPills not found');
}
html = html.replace(targetSnkPills, replacementSnkPills);
console.log('✓ Replaced sneaker angle pills');

// 4. PROJECT 3 HTML: Hardware stage HUD, viewport, and slider bar
...
if (!html.includes(targetHwStage)) {
  throw new Error('targetHwStage not found');
}
html = html.replace(targetHwStage, replacementHwStage);
console.log('✓ Replaced hardware stage HTML');

// Save intermediate state
fs.writeFileSync(srcFile, html);
console.log('Saved intermediate HTML markup changes.');
